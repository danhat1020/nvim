...
for _, plugin in ipairs(vim.pack.get()) do
    if not plugin.active then vim.pack.del({ plugin.spec.name }) end
end
-- plugins
require('transparent').setup({ extra_groups = { 'Pmenu', 'PmenuKind', 'PmenuExtra', 'FloatBorder' } })
...
vim.api.nvim_set_hl(0, 'StatusLine', { fg = '#a0a0a0', bg = 'none' })
-- set keymaps
vim.g.mapleader = ' '
...
local function LspOptions(args)
...
vim.api.nvim_create_autocmd('LspAttach', { callback = LspOptions })
